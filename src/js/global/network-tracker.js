...
window.addEventListener('apiCall', (e) => {
    if (e.detail.request._url.match(/api\/v1\/media\/\d*\/info\//)) {
        ...
        if (edges['carousel_media']) {
            ...
            const media = {
                url:
                    edges['media_type'] === 1
                        ? edges['image_versions2'].candidates[0]['url']
                        : edges['video_versions'][0].url,
    ...
    if (
        fbApiReqFriendlyName === 'PolarisStoriesV3ReelPageGalleryQuery' ||
    ...
    if (fbApiReqFriendlyName === 'PolarisStoriesV3HighlightsPageQuery') {
        ...
        nodes.forEach((node) => {
            ...
            window.dispatchEvent(
                new CustomEvent('highlightsLoad', {
                    detail: {
